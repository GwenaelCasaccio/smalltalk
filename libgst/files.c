...
void _gst_smalltalk_args(int argc, const char **argv) {
  smalltalk_argc = argc;
  smalltalk_argv = argv;
}

int _gst_initialize(const char *kernel_dir, const char *image_file, int flags) {
  ...
  site_pre_image_file =
      _gst_find_file(SITE_PRE_IMAGE_FILE_NAME, GST_DIR_KERNEL_SYSTEM);
