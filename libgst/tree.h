...
extern int _gst_list_length(tree_node listExpr) ATTRIBUTE_HIDDEN;
...
/* Create a method_node with the given fields (see description under
   ...
   ATTRIBUTES.  */
extern tree_node _gst_make_method(YYLTYPE *startLocation, YYLTYPE *endLocation,
...
extern tree_node
_gst_make_unary_expr(YYLTYPE *location, tree_node receiver,
                     const char *unarySelectorExpr) ATTRIBUTE_HIDDEN;
...
/* Create an expr_node to be passed to _gst_make_method for
   ...
   ARGUMENT.  */
extern tree_node _gst_make_binary_expr(YYLTYPE *location, tree_node receiver,
                                       const char *binaryOp,
                                       tree_node argument) ATTRIBUTE_HIDDEN;
...
extern tree_node
_gst_make_keyword_expr(YYLTYPE *location, tree_node receiver,
                       tree_node keywordMessage) ATTRIBUTE_HIDDEN;
...
extern tree_node _gst_make_keyword_list(YYLTYPE *location, const char *keyword,
...
extern tree_node _gst_make_attribute_list(YYLTYPE *location,
                                          tree_node constant) ATTRIBUTE_HIDDEN;
...
extern tree_node _gst_make_variable_list(YYLTYPE *location,
                                         tree_node variable) ATTRIBUTE_HIDDEN;
...
extern tree_node _gst_make_assign(YYLTYPE *location, tree_node variables,
                                  tree_node expression) ATTRIBUTE_HIDDEN;
...
extern tree_node _gst_make_int_constant(YYLTYPE *location,
                                        intptr_t ival) ATTRIBUTE_HIDDEN;

/* Create a const_node storing a double, FVAL.  The type (FloatD,
   FloatE, FloatQ) is given by TYPE.  */
extern tree_node _gst_make_float_constant(YYLTYPE *location, long double fval,
...
_gst_make_deferred_binding_constant(YYLTYPE *location,
                                    tree_node varNode) ATTRIBUTE_HIDDEN;
...
_gst_make_array_constructor(YYLTYPE *location,
                            tree_node statements) ATTRIBUTE_HIDDEN;
...
_gst_make_binding_constant(YYLTYPE *location,
                           tree_node variables) ATTRIBUTE_HIDDEN;
...
extern tree_node _gst_intern_ident(YYLTYPE *location,
                                   const char *ident) ATTRIBUTE_HIDDEN;
...
extern tree_node _gst_make_array_elt(YYLTYPE *location,
                                     tree_node elt) ATTRIBUTE_HIDDEN;

/* Creates a block tree node with the given ARGUMENTS, TEMPORARIES and
   STATEMENTS.  */
extern tree_node _gst_make_block(YYLTYPE *location, tree_node arguments,
                                 tree_node temporaries,
                                 tree_node statements) ATTRIBUTE_HIDDEN;
...
extern tree_node
_gst_make_cascaded_message(YYLTYPE *location, tree_node messageExpr,
                           tree_node cascadedMessages) ATTRIBUTE_HIDDEN;
...
extern tree_node _gst_make_message_list(YYLTYPE *location,
                                        tree_node messageElt) ATTRIBUTE_HIDDEN;
...
extern tree_node _gst_add_node(tree_node n1, tree_node n2) ATTRIBUTE_HIDDEN;

/* Free the objects on the compilation obstack.  */
extern void _gst_free_tree() ATTRIBUTE_HIDDEN;

/* Print the NODE with LEVEL spaces of indentation.  */
extern void _gst_print_tree(tree_node node, int level) ATTRIBUTE_HIDDEN;
