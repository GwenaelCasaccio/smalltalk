 ...
 *Inc. Written by Paolo Bonzini.  Ideas based on Mike Haertel's malloc.
 *
...
#define SMALL2FREE(B, N)                                                       \
  ((heap_freeobj *)(((char *)(B)->vSmall.data) + (N) * (B)->size))
...
#define offset_of(field, type) (((char *)&(((type *)8)->field)) - (char *)8)
...
static void init_heap(heap_data *h, size_t heap_allocation_size,
                      size_t heap_limit);
...
PTR _gst_mem_alloc(heap_data *h, size_t sz) {
  size_t lnr;
  heap_freeobj *mem;
  heap_block **mptr;
  heap_block *blk;
...
PTR _gst_mem_realloc(heap_data *h, PTR mem, size_t size) {
  heap_block *info;
  int pages_to_free;
  unsigned mmap_block;
...
void _gst_mem_free(heap_data *h, PTR mem) {
  heap_block *info;
  heap_freeobj *obj;
  int lnr;
  int msz;
...
/* Allocate a new block of memory.  The block will contain 'nr' objects
   each of 'sz' bytes.  */
static heap_block *heap_small_block(heap_data *h, size_t sz) {
  heap_block *info;
  int i;
  int nr;
...
static heap_block *heap_large_block(heap_data *h, size_t sz) {
  heap_block *info;
  size_t msz;
...
/* Allocate a block of memory from the free list or, failing that, the
   system pool.  */
static heap_block *heap_primitive_alloc(heap_data *h, size_t sz) {
  heap_block *ptr;
  heap_block **pptr;
...
static void heap_primitive_free(heap_data *h, heap_block *mem) {
#ifndef OPTIMIZE
