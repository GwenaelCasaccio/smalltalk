...
extern const char *_gst_kernel_file_path ATTRIBUTE_HIDDEN;
extern const char *_gst_image_file_path ATTRIBUTE_HIDDEN;
...
/* This is the name of the binary image to load.  If it is not NULL after the
   ...
   default_image_name.  */
extern const char *_gst_binary_image_name ATTRIBUTE_HIDDEN;
...
extern mst_Boolean _gst_regression_testing ATTRIBUTE_HIDDEN;
...
/* The standard value of this flag is 2.  The four possible values (0 to 3)
   ...
   in a cgi-bin script).  */
extern int _gst_verbosity ATTRIBUTE_HIDDEN;

/* The argc and argv that are made available to Smalltalk programs
   through the -a option.  */
extern int _gst_smalltalk_passed_argc ATTRIBUTE_HIDDEN;
extern const char **_gst_smalltalk_passed_argv ATTRIBUTE_HIDDEN;
...
extern mst_Boolean _gst_kernel_initialized ATTRIBUTE_HIDDEN;

/* Set by cmd line flag.  If true, Smalltalk is more verbose about
   what it's doing.  */
extern mst_Boolean _gst_verbose ATTRIBUTE_HIDDEN;

/* This is true if the image initialization has already been
   started.  */
extern mst_Boolean _gst_smalltalk_initialized ATTRIBUTE_HIDDEN;

/* This sets the arguments to be passed to the Smalltalk library,
   which are the same that are available by the `gst' executable.  */
extern void _gst_smalltalk_args(int argc, const char **argv) ATTRIBUTE_HIDDEN;

/* Set the fundamental paths for the Smalltalk VM and initialize
   it.  */
extern int _gst_initialize(const char *kernel_dir, const char *image_file,
...
extern void _gst_top_level_loop(void) ATTRIBUTE_HIDDEN;
...
extern mst_Boolean _gst_omit_line_numbers ATTRIBUTE_HIDDEN;
