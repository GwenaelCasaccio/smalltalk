...
struct heap_data {
  ...
  allocating_hook_t after_allocating, before_prim_freeing,
      after_prim_allocating;
...
extern PTR _gst_mem_alloc(heap_data *h, size_t n) ATTRIBUTE_HIDDEN;

/* Free the memory chunk pointed to by P, which was allocated from the
   independent heap H.  */
extern void _gst_mem_free(heap_data *h, PTR p) ATTRIBUTE_HIDDEN;
...
/* Resize the memory chunk pointed to by P, which was allocated from the
   ...
   returned.  */
extern PTR _gst_mem_realloc(heap_data *h, PTR p, size_t n) ATTRIBUTE_HIDDEN;
...
extern PTR xrealloc(PTR p, size_t n) ATTRIBUTE_HIDDEN;
...
/* Allocate memory for a copy of the null-terminated string S using malloc,
   ...
   if the memory cannot be allocated.  */
extern char *xstrdup(const char *s) ATTRIBUTE_HIDDEN;
