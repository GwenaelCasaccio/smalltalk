...
extern unsigned long _gst_saved_bytecode_counter ATTRIBUTE_HIDDEN;
...
/* When not NULL, this causes the byte code interpreter to immediately
   ...
   top.  */
extern const char *_gst_abort_execution ATTRIBUTE_HIDDEN;
...
extern OOP _gst_prepare_execution_environment(void) ATTRIBUTE_HIDDEN;
...
extern OOP _gst_nvmsg_send(OOP receiver, OOP selector, OOP *args,
                           int nArgs) ATTRIBUTE_HIDDEN;

/* Start the interpreter, and go on until we terminate PROCESSOOP.  */
extern OOP _gst_interpret(OOP processOOP) ATTRIBUTE_HIDDEN;
...
/* Set up so that ENTRY->FUNC will be called, with ENTRY->DATA as its
   ...
   safe version.  */
extern void _gst_async_call_internal(async_queue_entry *entry) ATTRIBUTE_HIDDEN;
...
/* Signal SEMAPHOREOOP so that one of the processes waiting on that
   ...
   is reached.  */
extern void _gst_async_signal(OOP semaphoreOOP) ATTRIBUTE_HIDDEN;
...
/* Signal SEMAPHOREOOP so that one of the processes waiting on that
   ...
   as soon as the next sequence point is reached.  */
extern void _gst_async_signal_and_unregister(OOP semaphoreOOP) ATTRIBUTE_HIDDEN;

/* Invalidate all the cached CompiledMethod lookups.  This does NOT
   include inline caches when the JIT compiler is active.  */
extern void _gst_invalidate_method_cache(void) ATTRIBUTE_HIDDEN;

/* Show a backtrace of the current state of the stack of execution
   contexts.  */
extern void _gst_show_backtrace(FILE *) ATTRIBUTE_HIDDEN;

/* Trap the signals that we care about, basically SIGBUS and
   SIGSEGV.  */
extern void _gst_init_signals(void) ATTRIBUTE_HIDDEN;
...
extern void _gst_fixup_object_pointers(void) ATTRIBUTE_HIDDEN;
...
/* Complementary to _gst_fixup_object_pointers, this function picks
   ...
   grown.  */
extern void _gst_restore_object_pointers(void) ATTRIBUTE_HIDDEN;
...
/* This runs before every evaluation and before GC turned on.  It creates an
   ...
   created yet.  */
extern void _gst_init_process_system(void) ATTRIBUTE_HIDDEN;
...
/* These function mark or copy all the objects that the interpreter keeps in
   ...
   dictionary.  */
extern void _gst_mark_processor_registers(void) ATTRIBUTE_HIDDEN;
extern void _gst_copy_processor_registers(void) ATTRIBUTE_HIDDEN;

/* Print the current state of the lists of ready to run processes for
   each priority, for debugging purposes.  */
extern void _gst_print_process_state(void) ATTRIBUTE_HIDDEN;

/* Sanity check the process lists that the sole instance of ProcessorScheduler
   holds.  */
extern void _gst_check_process_state(void) ATTRIBUTE_HIDDEN;

/* Print the objects currently on the stack, for debugging
   purposes.  */
extern void _gst_show_stack_contents(void) ATTRIBUTE_HIDDEN;
...
/* Called after the mark phase, but before the sweep phase, so that if
   ...
   are actually freed.  */
extern void _gst_validate_method_cache_entries(void) ATTRIBUTE_HIDDEN;

/* Terminate execution of the given PROCESSOOP.  */
extern void _gst_terminate_process(OOP processOOP) ATTRIBUTE_HIDDEN;
...
extern void _gst_send_method(OOP methodOOP) ATTRIBUTE_HIDDEN;
...
extern void _gst_sync_wait(OOP semaphoreOOP) ATTRIBUTE_HIDDEN;
...
extern OOP _gst_make_block_closure(OOP blockOOP) ATTRIBUTE_HIDDEN;
...
typedef intptr_t (*primitive_func)(int primitive, volatile int numArgs);
...
_gst_get_primitive_attributes(int primitive) ATTRIBUTE_PURE ATTRIBUTE_HIDDEN;
...
extern void _gst_set_primitive_attributes(int primitive, prim_table_entry *pte)
    ATTRIBUTE_HIDDEN;

/* Initialize the table of primitives.  */
extern void _gst_init_primitives() ATTRIBUTE_HIDDEN;

/* Get the value of internal variable whose number is INDEX; the
   list of valid variables is in gstpub.h.  Return -1 if the index
   is invalid.  */
extern int _gst_get_var(enum gst_var_index index) ATTRIBUTE_HIDDEN;
...
/* Set the value of internal variable whose number is INDEX; the
   ...
   value.  */
extern int _gst_set_var(enum gst_var_index index, int value) ATTRIBUTE_HIDDEN;
