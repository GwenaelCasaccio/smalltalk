...
static void parse_variable_name(const char **pp, const char **endp);
...
/* This creates a Symbol containing LEN bytes starting at STR and puts
   ...
   found.  */
static OOP intern_counted_string(const char *str, int len);
...
static const symbol_info sym_info[] = {
    {&_gst_and_symbol, "and:"},
    {&_gst_as_scaled_decimal_radix_scale_symbol,
...
const char *_gst_get_scope_kind(scope_type scope) {
  switch (scope) {
...
int _gst_declare_name(const char *name, bool writeable, bool allowDup) {
  symbol_list newList;
...
static void free_scope_symbols(scope scope) {
  ...
  for (oldList = scope->symbols; oldList != NULL; oldList = scope->symbols) {
...
static pool_list *combine_local_pools(OOP sharedPoolsOOP,
...
static void visit_pool(OOP poolOOP, struct pointer_set_t *grey,
                       struct pointer_set_t *white, pool_list *p_head,
                       pool_list *p_tail) {
  pool_list entry;
  ...
  if (is_a_kind_of(OOP_CLASS(poolOOP), _gst_abstract_namespace_class)) {
    ...
    if (!IS_NIL(OBJ_NAMESPACE_GET_SUPER_SPACE(pool))) {
      visit_pool(OBJ_NAMESPACE_GET_SUPER_SPACE(pool), grey, white, p_head,
                 p_tail);
    }
  }
...
void _gst_compute_linearized_pools(gst_parser *parser, bool forDoit) {
  ...
  for (classOOP = myClass; !IS_NIL(classOOP); classOOP = SUPERCLASS(classOOP)) {
    ...
    p_end = add_shared_pool_resolution(
        classOOP, forDoit ? environmentOOP : CLASS_ENVIRONMENT(classOOP),
        p_end);
  }
}

OOP find_class_variable(OOP varName) {
  pool_list pool;
  OOP assocOOP;
...
OOP _gst_push_temporaries_dictionary(void) {
...
void _gst_pop_temporaries_dictionary(OOP dictionaryOOP) {
  _gst_unregister_oop(temporaries_dictionary);
  temporaries_dictionary = dictionaryOOP;
}

tree_node _gst_find_variable_binding(tree_node list) {
...
bool _gst_find_variable(symbol_entry *se, tree_node list) {
  ...
  if (symbol == _gst_self_symbol || symbol == _gst_super_symbol) {
    fill_symbol_entry(se, SCOPE_SPECIAL, true, symbol, RECEIVER_INDEX, 0);
    return (true);
    ...
    fill_symbol_entry(se, SCOPE_SPECIAL, true, symbol, FALSE_INDEX, 0);
    return (true);
    ...
    fill_symbol_entry(se, SCOPE_SPECIAL, true, symbol, THIS_CONTEXT_INDEX, 0);
    return (true);
  }
  ...
  for (scope = _gst_compiler_state->cur_scope, scopeDistance = 0; scope != NULL;
    ...
    if (s) {
      fill_symbol_entry(se, SCOPE_TEMPORARY, s->readOnly, symbol, s->index,
                        scopeDistance);
      return (true);
    }
  }
...
void _gst_print_symbol_entry(symbol_entry *ent) {
  printf("%#O", ent->symbol);
  switch (ent->scope) {
  case SCOPE_RECEIVER:
...
OOP _gst_find_pragma_handler(OOP classOOP, OOP symbolOOP) {
  ...
  for (class_oop = myClass; !IS_NIL(class_oop);
    ...
    handlerOOP = _gst_identity_dictionary_at(
        OBJ_CLASS_GET_PRAGMA_HANDLERS(class), symbolOOP);
...
OOP _gst_make_instance_variable_array(OOP superclassOOP,
  ...
  if (IS_NIL(superclassOOP)) {
    ...
    superInstanceVars = numInstanceVars = NUM_OOPS(OOP_TO_OBJ(superArrayOOP));
  }
...
OOP _gst_make_class_variable_dictionary(const char *variableNames,
                                        OOP classOOP) {
...
OOP _gst_make_pool_array(const char *poolNames) {
  OOP poolsOOP, name;
  gst_object pools;
  int numPools, i;
  const char *p, *e;
  ...
  for (p = poolNames, i = 0; *p; i++) {
    ...
    if (!IS_NIL(name)) {
      ...
      pools->data[i] = dictionary_at(_gst_smalltalk_dictionary, name);
    }
  }
...
OOP _gst_intern_string_oop(OOP stringOOP) {
  unsigned int len;
...
OOP _gst_intern_string(const char *str) {
  int len;
...
static uintptr_t hash_symbol(const char *str, int len) {
...
int _gst_string_oop_len(OOP oop) {
...
#ifdef HAVE_READLINE
void _gst_add_all_symbol_completions(void) {
  int i;
...
int _gst_selector_num_args(OOP symbolOOP) {
  char *bytes;
  int numArgs, len;
...
void _gst_init_symbols_pass1(void) {
  ...
  for (size_t i = 0; i < sizeof(_gst_builtin_selectors_hash) /
