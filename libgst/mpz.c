...
#define MPN_ZERO(DST, NLIMBS) memset((DST), 0, (NLIMBS)*SIZEOF_MP_LIMB_T)
...
static void gst_mpz_sub_ui(gst_mpz *dif, const gst_mpz *min, mp_limb_t sub) {
  ...
  if (minsize < 0) {
    ...
    if (abs_minsize == 1 && minp[0] < sub) {
      difsize = -(abs_minsize + mpn_sub_1(difp, &sub, 1, *minp));
    } else {
      difsize = (abs_minsize + mpn_sub_1(difp, minp, abs_minsize, sub));
    }
  }
...
void _gst_mpz_add(gst_mpz *sum, const gst_mpz *u, const gst_mpz *v) {
  mp_srcptr up, vp;
  mp_ptr sump;
  ...
  if (usize >= 0) {
    if (vsize >= 0) {
      ...
      if (abs_usize == abs_vsize && mpn_cmp(up, vp, abs_usize) < 0) {
        sumsize = -(abs_usize + mpn_sub(sump, vp, abs_usize, up, abs_usize));
      } else {
        sumsize = (abs_usize + mpn_sub(sump, up, abs_usize, vp, abs_vsize));
      }
    }
  } else {
    if (vsize >= 0) {
...
int _gst_mpz_cmp(const gst_mpz *u, const gst_mpz *v) {
...
void _gst_mpz_div_2exp(gst_mpz *w, const gst_mpz *u, unsigned cnt) {
  mp_size_t usize = u->size;
  mp_size_t wsize;
...
void _gst_mpz_tdiv_qr(gst_mpz *quot, gst_mpz *rem, const gst_mpz *num,
                      const gst_mpz *den) {
  mp_ptr np, dp;
  mp_ptr qp, rp;
...
static void _gst_mpz_tdiv_q_ui(gst_mpz *quot, const gst_mpz *num,
                               mp_limb_t den) {
  mp_ptr np;
  mp_ptr qp;
  mp_size_t nsize = num->size;
...
void _gst_mpz_gcd(gst_mpz *g, const gst_mpz *u, const gst_mpz *v) {
...
void _gst_mpz_fdiv_qr(gst_mpz *quot, gst_mpz *rem, const gst_mpz *dividend,
                      const gst_mpz *divisor) {
...
mp_limb_t _gst_mpz_fdiv_qr_si(gst_mpz *quot, const gst_mpz *dividend,
                              intptr_t divisor) {
...
void _gst_mpz_mul(gst_mpz *w, const gst_mpz *u, const gst_mpz *v) {
  mp_size_t usize = u->size;
  mp_size_t vsize = v->size;
  mp_size_t wsize;
...
void _gst_mpz_mul_2exp(gst_mpz *w, const gst_mpz *u, unsigned cnt) {
  mp_size_t usize = u->size;
  mp_size_t abs_usize = ABS(usize);
  mp_size_t wsize;
  mp_size_t limb_cnt;
  ...
  if (cnt % BITS_PER_MP_LIMB) {
    wdigit = mpn_lshift(wp + limb_cnt, u->d, abs_usize, cnt % BITS_PER_MP_LIMB);
    wsize = abs_usize + limb_cnt;
...
void _gst_mpz_set(gst_mpz *w, const gst_mpz *u) {
  mp_size_t usize;
  mp_size_t abs_usize;
...
void _gst_mpz_sub(gst_mpz *w, const gst_mpz *u, const gst_mpz *v) {
  mp_srcptr up, vp;
  mp_ptr wp;
...
void _gst_mpz_and(gst_mpz *res, const gst_mpz *op1, const gst_mpz *op2) {
  ...
  if (op1_size >= 0) {
    ...
    if (op2_size < 0) {
      ...
      {
        const mp_limb_t *t = op1_ptr;
        op1_ptr = op2_ptr;
        op2_ptr = t;
      }
      {
        const mp_size_t t = op1_size;
        op1_size = op2_size;
        op2_size = t;
      }
    }
...
void _gst_mpz_ior(gst_mpz *res, const gst_mpz *op1, const gst_mpz *op2) {
  mp_srcptr op1_ptr, op2_ptr;
  mp_size_t op1_size, op2_size;
  mp_ptr res_ptr;
  mp_size_t res_size;
...
void _gst_mpz_xor(gst_mpz *res, const gst_mpz *op1, const gst_mpz *op2) {
  mp_srcptr op1_ptr, op2_ptr;
  mp_size_t op1_size, op2_size;
  mp_ptr res_ptr;
...
void _gst_mpz_divexact(gst_mpz *quot, const gst_mpz *num, const gst_mpz *den) {
  mp_ptr qp;
  mp_srcptr np, dp;
...
OOP _gst_oop_from_mpz(gst_mpz *mpz) {
  ...
  if (neg) {
    ...
    ba = new_instance_with(_gst_large_negative_integer_class, ++n, &oop);
  } else {
    ...
    ba = new_instance_with(_gst_large_positive_integer_class, ++n, &oop);
  }
