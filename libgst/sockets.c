...
static char *myGetHostByAddr(char *addr, int len, int type) {
...
static char **get_aiCanonname(struct addrinfo *ai) { return &ai->ai_canonname; }

static struct sockaddr **get_aiAddr(struct addrinfo *ai) {
  return &ai->ai_addr;
}
...
constantFunction(aiAddrconfig, AI_ADDRCONFIG)
    constantFunction(aiCanonname, AI_CANONNAME) constantFunction(aiAll, AI_ALL)
...
static int myConnect(int fd, struct sockaddr *sockaddr, int len) {
...
static int myBind(int fd, struct sockaddr *addr, socklen_t addrlen) {
  fix_sockaddr(addr, addrlen);
  return bind(FD_TO_SOCKET(fd), addr, addrlen);
}

static int myGetpeername(int fd, struct sockaddr *addr, socklen_t *addrlen) {
  /* Parameters to system calls are not guaranteed to generate a SIGSEGV
     and for this reason we must touch them manually.  */
  _gst_grey_oop_range(addr, *addrlen);
...
static int myGetsockname(int fd, struct sockaddr *addr, socklen_t *addrlen) {
  /* Parameters to system calls are not guaranteed to generate a SIGSEGV
     and for this reason we must touch them manually.  */
  _gst_grey_oop_range(addr, *addrlen);
...
static int myGetsockopt(int fd, int level, int optname, char *optval,
                        socklen_t *optlen) {
  /* Parameters to system calls are not guaranteed to generate a SIGSEGV
     and for this reason we must touch them manually.  */
...
static int myRecvfrom(int fd, char *buf, int len, int flags,
...
static int mySendto(int fd, const char *buf, int len, int flags,
                    struct sockaddr *to, int tolen) {
  fix_sockaddr(to, tolen);
  return sendto(FD_TO_SOCKET(fd), buf, len, flags, to, tolen);
}

static int mySetsockopt(int fd, int level, int optname, const char *optval,
                        int optlen) {
