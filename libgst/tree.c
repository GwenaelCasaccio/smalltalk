...
static inline tree_node make_list_node(YYLTYPE *location, node_type nodeType,
                                       const char *name, tree_node value);

/* Make a expr_node made up of the NODETYPE type-tag and an expr_node
   with given RECEIVER, SELECTOR and EXPRESSION.  */
static inline tree_node make_expr_node(YYLTYPE *location, node_type nodeType,
                                       tree_node receiver, OOP selector,
                                       tree_node expression);

/* Allocate from the compilation obstack a node and assign it the
   NODETYPE type-tag.  */
static inline tree_node make_tree_node(YYLTYPE *location, node_type nodeType);

/* Print the NODE method_node with LEVEL spaces of indentation.  */
static void print_method_node(tree_node node, int level);

/* Print the NODE method_node with LEVEL spaces of indentation.  */
static void print_block_node(tree_node node, int level);

/* Print the NODE expr_node with LEVEL spaces of indentation.  */
static void print_expr_node(tree_node node, int level);

/* Print the NODE list_node with LEVEL spaces of indentation.  */
static void print_list_node(tree_node node, int level);
...
static void print_const_node(tree_node node, int level);

/* Print the NODE list_node with LEVEL spaces of indentation,
   discarding the NAME of each node (hence the distinction between
   this and print_list_node).  */
static void print_array_constructor_node(tree_node node, int level);

/* Print LEVEL spaces of indentation.  */
static void indent(int level);

int _gst_list_length(tree_node listExpr) {
  tree_node l;
  long len;
...
tree_node _gst_make_array_elt(YYLTYPE *location, tree_node elt) {
  return (make_list_node(location, TREE_ARRAY_ELT_LIST, NULL, elt));
}

tree_node _gst_make_method(YYLTYPE *location, YYLTYPE *endLocation,
...
tree_node _gst_make_cascaded_message(YYLTYPE *location, tree_node messageExpr,
                                     tree_node cascadedMessages) {
  return (make_expr_node(location, TREE_CASCADE_EXPR, messageExpr, NULL,
                         cascadedMessages));
}

tree_node _gst_make_unary_expr(YYLTYPE *location, tree_node receiver,
...
tree_node _gst_intern_ident(YYLTYPE *location, const char *ident) {
  return (make_expr_node(location, TREE_SYMBOL_NODE, NULL,
                         _gst_intern_string(ident), NULL));
}

tree_node _gst_make_return(YYLTYPE *location, tree_node expression) {
  return (make_expr_node(location, TREE_RETURN_EXPR, expression, _gst_nil_oop,
                         NULL));
}

tree_node _gst_make_keyword_expr(YYLTYPE *location, tree_node receiver,
                                 tree_node keywordMessage) {
  return (make_expr_node(location, TREE_KEYWORD_EXPR, receiver, _gst_nil_oop,
                         keywordMessage));
}

tree_node _gst_make_assign(YYLTYPE *location, tree_node variables,
                           tree_node expression) {
  return (make_expr_node(location, TREE_ASSIGN_EXPR, variables, _gst_nil_oop,
                         expression));
}

tree_node _gst_make_statement_list(YYLTYPE *location, tree_node expression) {
  return (make_list_node(location, TREE_STATEMENT_LIST, NULL, expression));
}

tree_node _gst_make_attribute_list(YYLTYPE *location, tree_node constant) {
  return (make_list_node(location, TREE_ATTRIBUTE_LIST, NULL, constant));
}

tree_node _gst_make_keyword_list(YYLTYPE *location, const char *keyword,
                                 tree_node expression) {
...
tree_node _gst_make_assignment_list(YYLTYPE *location, tree_node variable) {
  return (make_list_node(location, TREE_VAR_ASSIGN_LIST, NULL, variable));
}

tree_node _gst_make_binary_expr(YYLTYPE *location, tree_node receiver,
                                const char *binaryOp, tree_node argument) {
  OOP selector;

  selector = _gst_intern_string(binaryOp);
  return (
      make_expr_node(location, TREE_BINARY_EXPR, receiver, selector, argument));
}

tree_node _gst_make_message_list(YYLTYPE *location, tree_node messageElt) {
  return (make_list_node(location, TREE_MESSAGE_LIST, NULL, messageElt));
}

tree_node _gst_make_block(YYLTYPE *location, tree_node arguments,
                          tree_node temporaries, tree_node statements) {
  tree_node result;
...
tree_node _gst_make_variable(YYLTYPE *location, const char *name) {
  return (make_list_node(location, TREE_VARIABLE_NODE, name, NULL));
}

tree_node _gst_make_int_constant(YYLTYPE *location, intptr_t ival) {
  tree_node result;
...
tree_node _gst_make_byte_object_constant(YYLTYPE *location, byte_object boval) {
  tree_node result;
...
tree_node _gst_make_float_constant(YYLTYPE *location, long double fval,
                                   int type) {
  tree_node result;
...
tree_node _gst_make_string_constant(YYLTYPE *location, const char *sval) {
  tree_node result;
...
tree_node _gst_make_deferred_binding_constant(YYLTYPE *location,
                                              tree_node varNode) {
  tree_node result;
...
tree_node _gst_make_oop_constant(YYLTYPE *location, OOP oval) {
  tree_node result;
...
tree_node _gst_make_char_constant(YYLTYPE *location, int ival) {
  tree_node result;
...
tree_node _gst_make_symbol_constant(YYLTYPE *location, tree_node symbolNode) {
  tree_node result;
 ...
 * elements) to a gst_byte_array constant's format (byte_object struct).  The
 * code itself is awful and the list is extremely space inefficient, but
 * consider that: a) it makes the parser simpler (Arrays and ByteArrays are
 * treated in almost the same way; only, the latter call this function and the
 * former don't). b) a list is indeed an elegant solution because we don't know
 * the size of the byte array until we have parsed it all (that is, until we
 * call this function. c) the byte_object is the best format for ByteArrays:
 * first, it is the one which makes it easiest to make a full-fledged object out
 * of the parse tree; second, it is logical to choose it since LargeIntegers use
 * it, and ByteArrays are represented exactly the same as LargeIntegers.
 */
tree_node _gst_make_byte_array_constant(YYLTYPE *location, tree_node aval) {
...
tree_node _gst_make_array_constant(YYLTYPE *location, tree_node aval) {
  tree_node result;
...
tree_node _gst_make_array_constructor(YYLTYPE *location, tree_node statements) {
  tree_node result;
...
tree_node _gst_make_binding_constant(YYLTYPE *location, tree_node variables) {
  tree_node result;
...
static tree_node make_list_node(YYLTYPE *location, node_type nodeType,
                                const char *name, tree_node value) {
  tree_node result;
...
static tree_node make_expr_node(YYLTYPE *location, node_type nodeType,
                                tree_node receiver, OOP selector,
                                tree_node expression) {
  tree_node result;
...
static tree_node make_tree_node(YYLTYPE *location, node_type nodeType) {
  tree_node result;
 ...
 *
 ***********************************************************************/
void _gst_print_tree(tree_node node, int level) {
  const char *name;
  if (node == NULL) {
    printf("(nil)\n");
    return;
...
static void indent(int level) { printf("%*s", level, ""); }
