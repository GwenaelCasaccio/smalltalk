...
extern OOP _gst_invoke_croutine(OOP cFuncOOP, OOP receiver,
                                OOP *args) ATTRIBUTE_HIDDEN;
...
extern void _gst_define_cfunc(const char *funcName,
                              PTR funcAddr) ATTRIBUTE_HIDDEN;

/* Adds to the mapping table the standard C functions supported by
   GNU Smalltalk.  */
extern void _gst_init_cfuncs(void) ATTRIBUTE_HIDDEN;

/* Set the value of errno which is checked by Smalltalk to be errnum.  */
extern void _gst_set_errno(int errnum) ATTRIBUTE_HIDDEN;
...
extern PTR _gst_lookup_function(const char *funcName) ATTRIBUTE_HIDDEN;

/* Creates a closure for the CCallbackDescriptor CALLBACKOOP and stores it
   in the object.  */
extern void _gst_make_closure(OOP callbackOOP) ATTRIBUTE_HIDDEN;

/* Frees the info for the closure in the CCallbackDescriptor CALLBACKOOP.  */
extern void _gst_free_closure(OOP callbackOOP) ATTRIBUTE_HIDDEN;
...
extern void _gst_dlpushsearchpath(void) ATTRIBUTE_HIDDEN;

/* Pop the saved search path into the current libltdl search path.  */
extern void _gst_dlpopsearchpath(void) ATTRIBUTE_HIDDEN;
