...
#define FLAGS_WRITTEN (LOCAL_ENDIANNESS_FLAG | LOCAL_SLOT_SIZE_FLAG)
...
static void full_write(int fd, PTR buffer, size_t size);

/* This function establishes a buffer of size NUMBYTES for writes.  */
static void buffer_write_init(int imageFd, int numBytes);
...
static inline PTR buffer_advance(int imageFd, int numBytes);

/* This function buffers reads from the image file whose descriptor
   is IMAGEFD.  Memory-mapped I/O is used is possible.  */
static void buffer_read(int imageFd, PTR data, int numBytes);
...
static void save_object(int imageFd, OOP oop);
...
/* This function inverts the endianness of SIZE long-words, starting at
   BUF.  */
static inline void fixup_byte_order(PTR buf, size_t size);
...
/* This function stores the header, HEADERP, of the image file into the file
   whose descriptor is IMAGEFD.  */
static void save_file_version(int imageFd, struct save_file_header *headerp);
...
#define OOP_ABSOLUTE(obj) ((OOP)((intptr_t)(obj) + ot_delta))
...
void save_to_fd(int imageFd) {
  ...
  buffer_write(imageFd, myOOPTable, sizeof(struct oop_s) * num_used_oops);
...
struct oop_s *make_oop_table_to_be_saved(struct save_file_header *header) {
  OOP oop;
...
void save_file_version(int imageFd, struct save_file_header *headerp) {
  ...
  memcpy(&headerp->prim_table_md5, _gst_primitives_md5,
...
bool load_file_version(int imageFd, save_file_header *headerp) {
  ...
  if UNCOMMON ((wrong_endianness =
    ...
    headerp->grow_threshold_percent =
        BYTE_INVERT(headerp->grow_threshold_percent);
...
void load_oop_table(int imageFd) {
...
char *load_normal_oops(int imageFd) {
  ...
  for (oop = _gst_mem.ot; oop < &_gst_mem.ot[_gst_mem.ot_size]; OOP_NEXT(oop)) {
    ...
    if (use_copy_on_write) {
      ...
      if UNCOMMON (wrong_endianness)
        fixup_byte_order(object, (flags & F_BYTE) ? OBJ_HEADER_SIZE_WORDS
                                                  : size / sizeof(PTR));
...
void fixup_object(OOP oop, gst_object dest, gst_object src, int numBytes) {
  OOP class_oop;
  ...
  if (OOP_GET_FLAGS(oop) & F_CONTEXT) {
    ...
    gst_object destSem = dest;
    gst_object srcSem = src;
    OOP linkOOP = OBJ_SEMAPHORE_GET_FIRST_LINK(srcSem);
...
void full_write(int fd, PTR buffer, size_t size) {
...
void buffer_write(int imageFd, PTR data, int numBytes) {
...
void buffer_fill(int imageFd) {
  buf_pos = 0;
  read(imageFd, buf, buf_size);
}

char *buffer_read_init(int imageFd, int numBytes) {
  struct stat st;
  fstat(imageFd, &st);
...
void buffer_read(int imageFd, PTR pdata, int numBytes) {
