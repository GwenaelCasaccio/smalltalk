...
void _gst_send_message_internal(OOP sendSelector, int sendArgs, OOP receiver,
  ...
  if (header.numArgs != (unsigned)sendArgs) {
    ...
    if (methodData->methodClassOOP == receiverClass)
      ...
      printf("%O(%O)>>%O\n", receiverClass, methodData->methodClassOOP,
             sendSelector);

    if (header.numArgs != (unsigned)sendArgs) {
      _gst_errorf("invalid number of arguments %d, expecting %d", sendArgs,
                  header.numArgs);
...
void _gst_send_method(OOP methodOOP) {
  int sendArgs;
  OOP receiver;
  method_header header;
  ...
  /* prepare new state */
  newContext = activate_new_context(header.stack_depth, sendArgs);
  OBJ_METHOD_CONTEXT_SET_FLAGS((gst_object)newContext,
...
void _gst_validate_method_cache_entries(void) {}
...
/* If we have a good quantity of registers, activate more caching mechanisms. */
  ...
  OOP my_nil_oop = _gst_nil_oop, my_true_oop = _gst_true_oop,
      my_false_oop = _gst_false_oop;
