...
/* Given a number of bytes "x", return the number of 32 bit words
   ...
   word boundary.  */
#define ROUNDED_WORDS(x) (((x) + sizeof(long) - 1) / sizeof(long))
...
typedef struct gst_object_memory {
  ...
  OOP bytesPerOOP, bytesPerOTE, edenSize, survSpaceSize, oldSpaceSize,
      fixedSpaceSize, edenUsedBytes, survSpaceUsedBytes, oldSpaceUsedBytes,
      ...
      numOTEs, numFreeOTEs, timeBetweenScavenges, timeBetweenGlobalGCs,
      timeBetweenGrowths, timeToScavenge, timeToCollect, timeToCompact,
      reclaimedBytesPerScavenge, tenuredBytesPerScavenge,
      reclaimedBytesPerGlobalGC, reclaimedPercentPerScavenge, allocFailures,
      allocMatches, allocSplits, allocProbes;
} * gst_object_memory;
...
struct memory_space {
  ...
  double reclaimedBytesPerScavenge, tenuredBytesPerScavenge,
...
extern int _gst_gc_running ATTRIBUTE_HIDDEN;

/* Finds and returns an instance of the class CLASS_OOP.  Returns "nil"
   if there are no instances present.  */
extern OOP _gst_find_an_instance(OOP class_oop) ATTRIBUTE_PURE ATTRIBUTE_HIDDEN;

/* Execute a two-way become operation between OOP1 and OOP2.  */
extern void _gst_swap_objects(OOP oop1, OOP oop2) ATTRIBUTE_HIDDEN;
...
extern void _gst_scavenge(void) ATTRIBUTE_HIDDEN;
...
extern void _gst_global_gc(int next_allocation) ATTRIBUTE_HIDDEN;
...
/* The incremental collector has done its job.  Update statistics,
   ...
   objects as alive.  */
extern void _gst_finished_incremental_gc(void) ATTRIBUTE_HIDDEN;

/* Finish the incremental sweep phase of the GC.  */
extern void _gst_finish_incremental_gc(void) ATTRIBUTE_HIDDEN;

/* Move all the object in survivor space to old space.  */
extern void _gst_tenure_all_survivors() ATTRIBUTE_HIDDEN;
...
/* Initialize the memory allocator.  The memory space is allocated,
   ...
   values.  */
extern void _gst_init_mem_default() ATTRIBUTE_HIDDEN;
...
extern void _gst_init_mem(size_t eden, size_t survivor, size_t old,
                          ...
                          int space_grow_rate) ATTRIBUTE_HIDDEN;
...
/* Dump the entire contents of the OOP table.  Mainly for debugging
   purposes.  */
extern void _gst_dump_oop_table() ATTRIBUTE_HIDDEN;
...
/* The almost-depth-first copying collector.  If survivor space is
   ...
   reasons.  */
extern void _gst_copy_an_oop(OOP oop) ATTRIBUTE_HIDDEN;

/* Mark the objects pointed to by the list of pointers to OOPs at CUROOP
   (included) and finishing at ATENDOOP (excluded).  */
extern void _gst_mark_oop_range(OOP *curOOP, OOP *atEndOOP) ATTRIBUTE_HIDDEN;

/* Copy the objects pointed to by the list of pointers to OOPs at CUROOP
   (included) and finishing at ATENDOOP (excluded).  */
extern void _gst_copy_oop_range(OOP *curOOP, OOP *atEndOOP) ATTRIBUTE_HIDDEN;

/* Grey the pointers pointed to by the list of pointers to OOPs at FROM
   (included) and for SIZE bytes.  */
extern void _gst_grey_oop_range(PTR from, size_t size) ATTRIBUTE_HIDDEN;

/* Mark OOP and the pointers pointed by that.  */
extern void _gst_mark_an_oop_internal(OOP oop) ATTRIBUTE_HIDDEN;

/* Fully initialize the builtin objects, possible after the respective
   classes have been created.  */
extern void _gst_init_builtin_objects_classes(void) ATTRIBUTE_HIDDEN;

/* Create the registry of incubated objects.  */
extern void _gst_inc_init_registry(void) ATTRIBUTE_HIDDEN;

/* Grow the registry of incubated objects when it is full.  */
extern void _gst_inc_grow_registry(void) ATTRIBUTE_HIDDEN;
...
/* Allocate and return space for an object of SIZE bytes.  This
   ...
   stored in P_OOP.  */
extern gst_object _gst_alloc_obj(size_t size, OOP *p_oop) ATTRIBUTE_HIDDEN;
...
extern gst_object _gst_alloc_words(size_t size) ATTRIBUTE_HIDDEN;
...
extern void _gst_grow_memory_to(size_t size) ATTRIBUTE_HIDDEN;

/* Grow the OOP table to NEWSIZE pointers and initialize the newly
   created pointers.  */
extern mst_Boolean _gst_realloc_oop_table(size_t newSize) ATTRIBUTE_HIDDEN;

/* Move OOP to oldspace.  */
extern void _gst_tenure_oop(OOP oop) ATTRIBUTE_HIDDEN;

/* Move OOP to fixedspace.  */
extern void _gst_make_oop_fixed(OOP oop) ATTRIBUTE_HIDDEN;

/* Make OOP a weak object.  */
extern void _gst_make_oop_weak(OOP oop) ATTRIBUTE_HIDDEN;

/* Make OOP a non-weak object.  */
extern void _gst_make_oop_non_weak(OOP oop) ATTRIBUTE_HIDDEN;

/* Clear the OOP data related to OOP */
extern void _gst_sweep_oop(OOP oop) ATTRIBUTE_HIDDEN;

/* Set the fields of the given ObjectMemory object */
extern void _gst_update_object_memory_oop(OOP oop) ATTRIBUTE_HIDDEN;
