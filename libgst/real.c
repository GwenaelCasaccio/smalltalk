...
static int rshift_sig(struct real *out, struct real *in, int delta) {
  ...
  if (rshift) {
    for (i = 0; i + delta < SIGSZ - 1; i++) {
      out->sig[i] =
          ((in->sig[i + delta + 1] << lshift) | (in->sig[i + delta] >> rshift));
      nonzero |= out->sig[i];
    }
...
static void normalize(struct real *out, struct real *in) {
  ...
  if (lshift) {
    for (i = SIGSZ; --i - delta >= 1;) {
      out->sig[i] =
          ((in->sig[i - delta] << lshift) | (in->sig[i - delta - 1] >> rshift));
    }
...
void _gst_real_from_int(struct real *out, int s) {
  memset(out, 0, sizeof(struct real));
  if (s < 0) {
    out->sign = -1;
    s = -s;
...
void _gst_real_add(struct real *r, struct real *s) {
  if (!s->sign) {
    return;
  }
  if (!r->sign) {
...
void _gst_real_add_int(struct real *r, int s) {
  struct real s_real;
  if (!s) {
    return;
  }
...
void _gst_real_mul(struct real *r, struct real *s) {
  int i;
  struct real tmp;
  if (r->sign == 0) {
    return;
...
void _gst_real_mul_int(struct real *r, int s) {
  struct real s_real;
...
void _gst_real_powi(struct real *out, struct real *r, int s) {
  int k;
  struct real tmp;
  if (out == r) {
    tmp = *r;
...
void _gst_real_div(struct real *out, struct real *r, struct real *s) {
  assert(s->sign);
  if (!r->sign) {
    memset(out, 0, sizeof(struct real));
    return;
...
void _gst_real_inv(struct real *out, struct real *s) {
  struct real u;
  assert(s->sign);
...
long double _gst_real_get_ld(struct real *r) {
