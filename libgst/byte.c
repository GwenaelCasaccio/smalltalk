...
/* Reallocate an array of bytecodes, leaving space for DELTA more
   bytes.  */
static void realloc_bytecodes(bc_vector bytecodes, int delta);

bc_vector _gst_extract_bytecodes(OOP byteArrayOOP) {
  bc_vector result;
  int len;
...
static void compile_byte(gst_uchar byte, int arg) {
  int num_bytes;
  long n;
...
void _gst_line_number(int n, int flags) {
  if (n > 65535) {
    n = 65535;
  }
...
void _gst_compile_byte(gst_uchar byte, int arg) {
...
void _gst_free_bytecodes(bc_vector bytecodes) {
  if (bytecodes != NULL) {
...
void _gst_compile_and_free_bytecodes(bc_vector bytecodes) {
...
bc_vector _gst_get_bytecodes(void) {
...
bc_vector _gst_save_bytecode_array() {
  bc_vector curBytecodes;
...
void _gst_restore_bytecode_array(bc_vector bytecodes) {
  _gst_cur_bytecodes = bytecodes;
}

int _gst_bytecode_length(bc_vector bytecodes) {
  if (bytecodes == NULL) {
    return (0);
  }
...
int _gst_current_bytecode_length(void) {
...
void _gst_copy_bytecodes(gst_uchar *dest, bc_vector bytecodes) {
  memcpy(dest, bytecodes->base, _gst_bytecode_length(bytecodes));
}

void _gst_truncate_bytecodes(gst_uchar *here, bc_vector bytecodes) {
  bytecodes->ptr = here;
}

void _gst_print_bytecodes(bc_vector bytecodes, OOP *literal_vec) {
  gst_uchar *b;
  int ip;
...
gst_uchar *_gst_print_bytecode_name(gst_uchar *bp, int ip, OOP *literal_vec,
                                    const char *pref) {
  const char *prefix = "";
  gst_uchar b = *bp;

  MATCH_BYTECODES(
      PRINT_BYTECODE_NAME, bp,
      (
          ...
          POP_INTO_NEW_STACKTOP {
            printf("%s\tpop and store into Instance Variable[%d] of new stack "
                   "top\n",
                   prefix, n);
            prefix = pref;
          }
...
void _gst_compile_bytecodes(gst_uchar *from, gst_uchar *to) {
  int free;
  assert(_gst_cur_bytecodes);
...
void realloc_bytecodes(bc_vector bytecodes, int delta) {
  int size;
