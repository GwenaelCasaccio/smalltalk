...
static bool is_base_digit(int c, int base);
...
static int scan_fraction(int c, bool negative, unsigned base,
                         uintptr_t *intNumPtr, struct real *numPtr,
                         bool *largeInteger);
...
static uintptr_t scan_digits(int c, bool negative, unsigned base,
                             struct real *n, bool *largeInteger);
...
static byte_object scan_large_integer(bool negative, int base);

/* Raise an error.  */
static int invalid(int c, YYSTYPE *lvalp);

/* Parse a comment.  C is '"'.  Return 0 to indicate the lexer
   that this lexeme must be ignored.  */
static int comment(int c, YYSTYPE *lvalp);

/* Parse a character literal.  C is '$' */
static int char_literal(int c, YYSTYPE *lvalp);

/* Remember the current balance of open/close parentheses, used to treat
   newlines as periods.  */
static int scan_open_paren(int c, YYSTYPE *lvalp);

/* Remember the current balance of open/close parentheses, used to treat
   newlines as periods.  */
static int scan_close_paren(int c, YYSTYPE *lvalp);

/* Remember the current balance of open/close parentheses, used to treat
   newlines as periods.  */
static int scan_reset_paren(int c, YYSTYPE *lvalp);
...
int _gst_yylex(PTR lvalp, YYLTYPE *llocp) {
  int result;
...
int invalid(int c, YYSTYPE *lvalp) {
  UNUSED(lvalp);
...
int scan_reset_paren(int c, YYSTYPE *lvalp) {
  UNUSED(lvalp);
...
int scan_open_paren(int c, YYSTYPE *lvalp) {
  UNUSED(lvalp);
...
int scan_close_paren(int c, YYSTYPE *lvalp) {
  UNUSED(lvalp);
...
int scan_newline(int c, YYSTYPE *lvalp) {
  UNUSED(c);
  UNUSED(lvalp);
  ...
  if (_gst_get_cur_stream_prompt()) {
    ...
    if (parenthesis_depth == 0 && last_token != 0 && last_token != '.' &&
        last_token != '!' && last_token != KEYWORD && last_token != BINOP &&
        last_token != '|' && last_token != '<' && last_token != '>' &&
        last_token != ';' && last_token != ASSIGNMENT &&
        last_token != SCOPE_SEPARATOR) {
      return ('.');
    }
  }
...
int comment(int c, YYSTYPE *lvalp) {
  UNUSED(lvalp);
...
int char_literal(int c, YYSTYPE *lvalp) {
  UNUSED(c);
  UNUSED(lvalp);
  ...
  ic = _gst_next_char();
  if (ic == EOF) {
    _gst_errorf("Unterminated character literal, attempting recovery");
    _gst_unread_char(ic);
    _gst_had_error = true;
    return (0);
  } else {
    if (ic > 127) {
      _gst_errorf("Invalid character literal, only character codes from 0 to "
                  "127 are valid");
      _gst_had_error = true;
    }
    lvalp->ival = ic;
...
int scan_colon(int c, YYSTYPE *lvalp) {
  UNUSED(c);
  UNUSED(lvalp);
...
int scan_symbol(int c, YYSTYPE *lvalp) {
  UNUSED(c);
  UNUSED(lvalp);
  ...
  if (ic == '!') {
    ...
    if (loc.first_line == 1 && loc.first_column == 2) {
      while (((ic = _gst_next_char()) != EOF) && ic != '\r' && ic != '\n') {
        continue;
      }
      return (SHEBANG);
    }
...
int scan_bin_op_1(int c, YYSTYPE *lvalp, bool maybe_number) {
  UNUSED(maybe_number);
...
int scan_bin_op(int c, YYSTYPE *lvalp) { return scan_bin_op_1(c, lvalp, true); }

int string_literal(int c, YYSTYPE *lvalp) {
  int ic;
...
int scan_ident(int c, YYSTYPE *lvalp) {
  int ic, identType;
...
int scan_number(int c, YYSTYPE *lvalp) {
  OOP intNumOOP;
  ...
  if (ic == '.') {
    ...
    if (!is_base_digit(ic, base)) {
      ...
      exponent =
          scan_fraction(ic, isNegative, base, &intNum, &num, &largeInteger);
      ic = _gst_next_char();
    }
  }
    ...
    do {
      ...
      lvalp->oval =
          _gst_msg_send(intNumOOP, _gst_as_scaled_decimal_radix_scale_symbol,
                        FROM_INT(exponent), FROM_INT(base),
                        FROM_INT((int)floatExponent), NULL);
  ...
  if (ic == 'e' || ic == 'd' || ic == 'q') {
    ...
    switch (ic) {
    case 'e':
      float_type = FLOATE_LITERAL;
      break;
    case 'd':
      float_type = FLOATD_LITERAL;
      break;
    case 'q':
      float_type = FLOATQ_LITERAL;
      break;
    }
    ...
    if (ic == EOF) {
      ...
      floatExponent = scan_digits(_gst_next_char(), true, 10, &dummy, NULL);
...
uintptr_t scan_digits(int c, bool negative, unsigned base, struct real *n,
                      bool *largeInteger) {
  uintptr_t result;
  bool oneDigit = false;
...
int scan_fraction(int c, bool negative, unsigned base, uintptr_t *intNumPtr,
                  struct real *numPtr, bool *largeInteger) {
  uintptr_t intNum;
  int scale;
...
int digit_to_int(int c, int base) {
  if (c < '0' || (c > '9' && c < 'A') || c > 'Z') {
...
bool is_base_digit(int c, int base) {
  if (c < '0' || (c > '9' && c < 'A') || c > 'Z') {
    return (false);
  }
...
bool is_digit(int ic) {
  return (ic != EOF && (CHAR_TAB(ic)->char_class & DIGIT) != 0);
}

byte_object scan_large_integer(bool negative, int base) {
  int i;
  int size, digitsLeft;
  ...
  bo->class = negative ? _gst_large_negative_integer_class
...
OOP _gst_parse_method_from_stream(OOP currentClass, OOP currentCategory) {
...
void _gst_parse_stream(OOP currentNamespace) {
  struct obstack thisObstack, *oldObstack;
...
#ifdef LEXDEBUG
void print_token(token, yylval) int token;
YYSTYPE *yylval;
{
  switch (token) {
  case 0:
...
void _gst_yyprint(FILE *file, int token, PTR lval) {
...
bool _gst_negate_yylval(int token, YYSTYPE *yylval) {
  switch (token) {
  case INTEGER_LITERAL:
    if (yylval->ival < 0) {
      return false;
