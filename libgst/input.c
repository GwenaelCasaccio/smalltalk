...
static char *readline_quote_filename(const char *s, int rtype, const char *qcp);
static char *readline_dequote_filename(const char *s, char qc);
static char *symbol_generator(const char *text, int state);
static char **readline_match_symbols(char *text, int start, int end);
...
void _gst_push_unix_file(int fd, const char *fileName) {
...
void _gst_push_stream_oop(OOP oop) {
  input_stream newStream;
...
void _gst_push_smalltalk_string(OOP stringOOP) {
  input_stream newStream;
...
void _gst_push_cstring(const char *string) {
  input_stream newStream;
...
void _gst_push_stdin_string(void) {
#ifdef HAVE_READLINE
  input_stream newStream;
...
input_stream push_new_stream(stream_type type) {
  input_stream newStream;
...
void _gst_set_stream_info(int line, OOP fileOOP, OOP fileNameOOP,
                          int fileOffset) {
  in_stream->line = line;
  in_stream->column = 0;
...
void refill_stream(input_stream stream, char *buf, int new_line) {
...
int my_getc(input_stream stream) {
  int ic = 0;
  ...
  case STREAM_FILE:
    ...
    return (stream->st_file.ptr == stream->st_file.end)
               ? EOF
               : (unsigned char)*stream->st_file.ptr++;
...
OOP _gst_get_source_string(off_t startPos, off_t endPos) {
  ...
  if (startPos != -1 && !_gst_get_cur_stream_prompt()) {
    ...
    if (!IS_NIL(fileOOP)) {
      INC_ADD_OOP(fileOOP);
      fileSegment =
          (gst_file_segment)new_instance(_gst_file_segment_class, &result);
...
void _gst_warningf_at(int line, const char *str, ...) {
  va_list ap;
...
void _gst_warningf(const char *str, ...) {
  va_list ap;
...
void _gst_errorf_at(int line, const char *str, ...) {
  va_list ap;
...
void _gst_errorf(const char *str, ...) {
  va_list ap;
...
void _gst_yyerror(const char *s) { _gst_errorf("%s", s); }
...
int _gst_next_char(void) {
  int ic;

  if (in_stream->pushedBackCount > 0) {
    ic =
        (unsigned char)in_stream->pushedBackChars[--in_stream->pushedBackCount];
    return (ic);
  } else {
...
void _gst_enable_completion(void) { completions_enabled++; }

void _gst_disable_completion(void) { completions_enabled--; }

int poll_and_read(int fd, char *buf, int n) {
  int result;
...
char *readline_dequote_filename(const char *s, char qc) {
...
void add_completion(const char *str, int len) {
  ...
  if (!free_items) {
    free_items += 50;
    completions = (char **)xrealloc(completions, sizeof(char *) * (count + 50));
  }
...
void _gst_add_symbol_completion(const char *str, int len) {
  const char *base = str;
  const char *p = str;
...
int compare_strings(const PTR a, const PTR b) {
...
char *symbol_generator(const char *text, int state) {
  ...
  while (matches_left > 1 && strcmp(completions[current_index],
...
char **readline_match_symbols(char *text, int start, int end) {
  ...
  if (sorted_count < count) {
    qsort(&completions[sorted_count], count - sorted_count, sizeof(char *),
          compare_strings);

    merge(&completions[sorted_count], count - sorted_count, completions,
          sorted_count, true);
  ...
  return matches_left ? rl_completion_matches(text, symbol_generator) : NULL;
}
...
void _gst_initialize_readline(void) {
