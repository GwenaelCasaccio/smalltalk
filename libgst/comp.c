...
/* Returns true if OOP and CONSTEXPR represent the same literal value.
   ...
   literals.  */
static bool equal_constant(OOP oop, tree_node constExpr);
...
/* Special case compilation of a while loop whose selector is in
   ...
   much.  */
static bool compile_while_loop(OOP selector, tree_node expr);
...
static bool compile_if_statement(OOP selector, tree_node expr);
...
/* Special case compilation of a #to:do: (if BY is NULL) or #to:by:do:
   ...
   much.  */
static bool compile_to_by_do(tree_node to, tree_node by, tree_node block);
...
/* Special case compilation of a #and: or #or: boolean operation; very
   ...
   indicate as much.  */
static bool compile_and_or_statement(OOP selector, tree_node expr);
...
/* Special case compilation of a 2-argument if whose selector is in
   ...
   argument to the message is not a block expression, this routine
   cannot do its job, and so returns false to indicate as much.  */
static bool compile_if_true_false_statement(OOP selector, tree_node expr);
...
static bool compile_statements(tree_node statementList, bool isBlock);
...
static OOP method_new(method_header header, OOP literals, bc_vector bytecodes,
...
static OOP method_info_new(OOP class, OOP selector, method_attributes *attrs,
                           OOP sourceCode, OOP categoryOOP);
...
static OOP make_block(int args, int temps, bc_vector bytecodes,
                      int stack_depth);
...
/* Like compile_sub_expression, except that after compiling EXPR this
   ...
   BRANCHLEN bytecodes.  */
static bc_vector compile_sub_expression_and_jump(tree_node expr, int branchLen);

/* Compile a send with the given RECEIVER (used to check for sends to
   super), SELECTOR and number of arguments NUMARGS.  */
static void compile_send(tree_node receiver, OOP selector, int numArgs);
...
static void compile_unary_expr(tree_node expr, bool omit_receiver);
...
static void compile_binary_expr(tree_node expr, bool omit_receiver);
...
static void compile_keyword_expr(tree_node expr, bool omit_receiver);
...
/* Compiles a forward jump instruction LEN bytes away (LEN must be >
   ...
   interpreter handles.  */
static void compile_jump(int len, bool jumpType);
...
static void install_method(OOP methodOOP, OOP classOOP);
...
#define EXIT_COMPILATION() longjmp(_gst_compiler_state->bad_method, 1)
...
void _gst_install_initial_methods(void) {
  ...
  termination_method = _gst_make_new_method(
      0, 0, 0, _gst_nil_oop, _gst_nil_oop, _gst_get_bytecodes(),
      _gst_undefined_object_class, _gst_terminate_symbol,
      _gst_string_new("private"), -1, -1);
...
OOP _gst_get_termination_method(void) {
  if (!termination_method) {
    termination_method = _gst_find_class_method(_gst_undefined_object_class,
...
static void invoke_hook_smalltalk(enum gst_vm_hook hook) {
  ...
  _gst_msg_sendf(NULL, "%v %o changed: %S", _gst_object_memory_class,
                 hook_name);
}

void _gst_invoke_hook(enum gst_vm_hook hook) {
  int save_execution;
...
OOP _gst_execute_statements(OOP receiverOOP, tree_node method, bool undeclared,
                            bool quiet) {
  ...
  if (_gst_regression_testing || _gst_verbosity < 2 ||
  ...
  if (statements->v_list.value &&
    ...
    _gst_compiler_state->debugInfoDict =
        _gst_identity_dictionary_new(_gst_identity_dictionary_class, 6);
  ...
  if (!_gst_raw_profile) {
    _gst_bytecode_counter = _gst_primitives_executed = _gst_self_returns =
        _gst_inst_var_returns = _gst_literal_returns = _gst_sample_counter = 0;
  }
...
#ifdef ENABLE_JIT_TRANSLATION
        printf("%lu primitives, %lu inline cache misses since last cache "
               "cleanup\n",
...
OOP _gst_compile_method(tree_node method, bool returnLast, bool install,
  ...
  if (setjmp(_gst_compiler_state->bad_method) == 0) {
    ...
    if (argCount == -1) {
      _gst_errorf_at(method->location.first_line, "duplicate argument name");
      EXIT_COMPILATION();
    }
    ...
    methodOOP = _gst_make_new_method(
        _gst_get_arg_count(), _gst_get_temp_count(), stack_depth, attributesOOP,
        literalsOOP, bytecodes, method->v_method.currentClass, selector,
        method->v_method.currentCategory, method->location.file_offset,
    ...
    if (methodOOP != _gst_nil_oop) {
      ...
      if (args->nodeType == TREE_BINARY_EXPR) {
        object->data[i] =
            _gst_intern_string(args->v_expr.expression->v_list.name);
        i += 1;
      } else {
      ...
      for (args = method->v_method.temporaries; args != NULL;
           args = args->v_list.next) {
      ...
      _gst_identity_dictionary_at_put(_gst_compiler_state->debugInfoDict,
                                      methodOOP, debugInfo);
  ...
  if (methodOOP != _gst_nil_oop) {
    ...
    if (install) {
      install_method(methodOOP, _gst_curr_method->v_method.currentClass);
    }
  } else {
    _gst_had_error = true;
    bytecodes = _gst_get_bytecodes();
...
void compile_statement(tree_node stmt) {
  tree_node receiver;
...
void compile_expression(tree_node expr) {
...
void compile_simple_expression(tree_node expr) {
...
void compile_compile_time_constant(tree_node expr) {
  OOP resultOOP;
  tree_node constant;
...
void compile_variable(tree_node varName) {
  symbol_entry variable;

  INCR_STACK_DEPTH();
  if (!_gst_find_variable(&variable, varName)) {
    if (varName->v_list.next) {
      _gst_errorf_at(varName->location.first_line, "invalid scope resolution");
    } else {
      _gst_errorf_at(varName->location.first_line,
                     "undefined variable %s referenced", varName->v_list.name);
    }
    EXIT_COMPILATION();
  }
  ...
  if (variable.scope == SCOPE_SPECIAL) {
    ...
    case THIS_CONTEXT_INDEX: {
      ...
      if (!contextPartAssociation) {
        contextPartAssociation = dictionary_association_at(
            _gst_smalltalk_dictionary, _gst_intern_string("ContextPart"));
      }
...
void compile_constant(tree_node constExpr) {
  intptr_t intVal;
  int index = -1;
  OOP constantOOP;
  OOP *lit;
...
void compile_block(tree_node blockExpr) {
  ...
  if (argCount == -1) {
    _gst_errorf_at(blockExpr->location.first_line, "duplicate argument name");
    EXIT_COMPILATION();
  }
  ...
  object =
      new_instance_with(_gst_array_class, argCount + tempCount, &variablesOOP);
  INC_ADD_OOP(variablesOOP);

  for (i = 0, args = blockExpr->v_block.arguments; args != NULL;
       args = args->v_list.next) {
    object->data[i] = _gst_intern_string(args->v_list.name);
    i += 1;
  }

  for (args = blockExpr->v_block.temporaries; args != NULL;
       args = args->v_list.next) {
    object->data[i] = _gst_intern_string(args->v_list.name);
    i += 1;
  }
  ...
  _gst_identity_dictionary_at_put(_gst_compiler_state->debugInfoDict, blockOOP,
                                  debugInfo);
...
bool compile_statements(tree_node statementList, bool isBlock) {
  tree_node stmt;
...
void compile_array_constructor(tree_node arrayConstructor) {
  tree_node stmt, statementList;
  int i, n;
  static OOP arrayAssociation;

  statementList = arrayConstructor->v_const.val.aVal;
  for (n = 0, stmt = statementList; stmt; n++, stmt = stmt->v_list.next)
    ;

  if (!arrayAssociation) {
    arrayAssociation = dictionary_association_at(_gst_smalltalk_dictionary,
  ...
  for (i = 0, stmt = statementList; i < n; i++, stmt = stmt->v_list.next) {
    compile_statement(stmt->v_list.value);
...
void compile_unary_expr(tree_node expr, bool omit_receiver) {
  OOP selector;
...
void compile_binary_expr(tree_node expr, bool omit_receiver) {
  OOP selector;
...
void compile_keyword_expr(tree_node expr, bool omit_receiver) {
  OOP selector;
  int numArgs;
  ...
  if (!omit_receiver) {
    ...
    if (selector == _gst_if_true_symbol || selector == _gst_if_false_symbol) {
      ...
      if (compile_if_true_false_statement(selector, expr->v_expr.expression)) {
        return;
      }
    } else if (selector == _gst_and_symbol || selector == _gst_or_symbol) {
      ...
      if (compile_to_by_do(
              expr->v_expr.expression->v_list.value, NULL,
      ...
      if (compile_to_by_do(expr->v_expr.expression->v_list.value,
                           expr->v_expr.expression->v_list.next->v_list.value,
                           expr->v_expr.expression->v_list.next->v_list.next
                               ->v_list.value)) {
        return;
      }
    }
...
void compile_send(tree_node expr, OOP selector, int numArgs) {
  ...
  int super = expr->v_expr.receiver && is_super(expr->v_expr.receiver);
...
void compile_keyword_list(tree_node list) {
  for (; list; list = list->v_list.next) {
    compile_expression(list->v_list.value);
  }
}

bool compile_while_loop(OOP selector, tree_node expr) {
  ...
  if (expr->v_expr.receiver->nodeType != TREE_BLOCK_NODE ||
      (colon &&
       expr->v_expr.expression->v_list.value->nodeType != TREE_BLOCK_NODE)) {
    return (false);
  }

  if (HAS_PARAMS_OR_TEMPS(expr->v_expr.receiver) ||
      (colon && HAS_PARAMS_OR_TEMPS(expr->v_expr.expression->v_list.value))) {
    return (false);
  }
  ...
  for (oldJumpAroundLen = finalJumpLen = 0;; oldJumpAroundLen = jumpAroundLen) {
    ...
    finalJumpLen = (finalJumpOfs + finalJumpLen >= 65536 ? 6
                    : finalJumpOfs + finalJumpLen >= 256 ? 4
                                                         : 2);
    finalJumpLen = (finalJumpOfs + finalJumpLen >= 65536 ? 6
                    : finalJumpOfs + finalJumpLen >= 256 ? 4
                                                         : 2);
    jumpAroundLen = (jumpAroundOfs + finalJumpLen >= 65536 ? 6
                     : jumpAroundOfs + finalJumpLen >= 256 ? 4
                                                           : 2);
...
bool compile_repeat(tree_node receiver) {
  ...
  finalJumpLen = (repeatedLoopLen >= 65536 ? 6
                  : repeatedLoopLen >= 256 ? 4
                                           : 2);
  finalJumpLen = (repeatedLoopLen + finalJumpLen >= 65536 ? 6
                  : repeatedLoopLen + finalJumpLen >= 256 ? 4
                                                          : 2);
  finalJumpLen = (repeatedLoopLen + finalJumpLen >= 65536 ? 6
                  : repeatedLoopLen + finalJumpLen >= 256 ? 4
                                                          : 2);
...
bool compile_times_repeat(tree_node expr) {
...
bool compile_to_by_do(tree_node to, tree_node by, tree_node block) {
  int jumpAroundOfs, oldJumpAroundLen, jumpAroundLen;
  int finalJumpOfs, finalJumpLen;
  int index;
  ...
  if (by) {
    if (by->nodeType != TREE_CONST_EXPR || by->v_const.constType != CONST_INT) {
      return (false);
    }
  }

  index = _gst_declare_name(block->v_block.arguments->v_list.name, false, true);
  ...
  for (oldJumpAroundLen = jumpAroundLen = finalJumpLen = 0;;
  ...
  /* skip the loop if there are no more occurrences */
  _gst_compile_byte((!by || by->v_const.val.iVal > 0) ? GREATER_EQUAL_SPECIAL
                                                      : LESS_EQUAL_SPECIAL,
                    0);
  SUB_STACK_DEPTH(1);
  compile_jump(jumpAroundOfs + finalJumpLen, false);
...
bool compile_if_true_false_statement(OOP selector, tree_node expr) {
  ...
  if (selector == _gst_if_true_if_false_symbol) {
    falseByteCodes = compile_sub_expression(expr->v_list.next->v_list.value);
    trueByteCodes = compile_sub_expression_and_jump(
        expr->v_list.value, _gst_bytecode_length(falseByteCodes));
  } else {
    falseByteCodes = compile_sub_expression(expr->v_list.value);
    trueByteCodes = compile_sub_expression_and_jump(
        expr->v_list.next->v_list.value, _gst_bytecode_length(falseByteCodes));
  }
...
bool compile_if_statement(OOP selector, tree_node expr) {
  ...
  thenByteCodes = compile_sub_expression_and_jump(expr->v_list.value, 2);
...
bool compile_and_or_statement(OOP selector, tree_node expr) {
...
bc_vector compile_sub_expression(tree_node expr) {
  bool returns;
...
bc_vector compile_sub_expression_and_jump(tree_node expr, int branchLen) {
  bc_vector current_bytecodes, subExprByteCodes;
  bool returns;
...
void compile_jump(int len, bool jumpType) {
  if (len <= 0) {
...
void compile_cascaded_expression(tree_node expr) {
  _gst_line_number(expr->location.first_line, 0);
  switch (expr->nodeType) {
  case TREE_UNARY_EXPR:
...
void compile_cascaded_message(tree_node cascadedExpr) {
  tree_node message;
...
void compile_assignments(tree_node varList) {
  symbol_entry variable;
  ...
  for (; varList; varList = varList->v_list.next) {
    ...
    if (variable.readOnly) {
      _gst_errorf_at(varName->location.first_line,
                     ...
                     _gst_get_scope_kind(variable.scope), varName->v_list.name);
    ...
    if (variable.scopeDistance > 0) {
      _gst_compile_byte(STORE_OUTER_TEMP,
                        (variable.varIndex << 8) | variable.scopeDistance);
...
bool is_super(tree_node expr) {
  return (expr->nodeType == TREE_VARIABLE_NODE &&
          _gst_intern_string(expr->v_list.name) == _gst_super_symbol);
}

bool equal_constant(OOP oop, tree_node constExpr) {
  tree_node arrayElt;
  size_t len, i;
    ...
    if (IS_OOP(oop) && OOP_CLASS(oop) == _gst_array_class) {
      ...
      if (len == NUM_OOPS(OOP_TO_OBJ(oop))) {
        for (i = 1, arrayElt = constExpr; i <= len;
          ...
          if (!equal_constant(ARRAY_AT(oop, i), arrayElt->v_list.value)) {
            return (false);
          }
        }
        return (true);
  ...
  case CONST_DEFERRED_BINDING:
    if (IS_OOP(oop) && OOP_CLASS(oop) == _gst_deferred_variable_binding_class) {
      ...
      gst_object path =
          OOP_TO_OBJ(OBJ_DEFERRED_VARIABLE_BINDING_GET_PATH(binding));
      int i, size = NUM_OOPS(path);
      OOP *pKey;
      tree_node varNode = constExpr->v_const.val.aVal;

      /* Use <= because we test the key first.  */
      for (i = 0, pKey = &OBJ_DEFERRED_VARIABLE_BINDING_GET_KEY(binding);
           i <= size; pKey = &path->data[i++]) {
        if (!varNode || *pKey != _gst_intern_string(varNode->v_list.name)) {
          return (false);
        }
  ...
  case CONST_ARRAY:
    if (IS_OOP(oop) && OOP_CLASS(oop) == _gst_array_class) {
      ...
      for (len = 0, arrayElt = constExpr->v_const.val.aVal; arrayElt;
        ...
        ;
      if (len == NUM_OOPS(OOP_TO_OBJ(oop))) {
        for (i = 1, arrayElt = constExpr->v_const.val.aVal; i <= len;
             i++, arrayElt = arrayElt->v_list.next) {
          if (!equal_constant(ARRAY_AT(oop, i), arrayElt->v_list.value)) {
            return (false);
          }
...
OOP _gst_make_constant_oop(tree_node constExpr) {
  tree_node subexpr;
  int len, i;
  ...
  case CONST_DEFERRED_BINDING: {
    ...
    OBJ_DEFERRED_VARIABLE_BINDING_SET_KEY(
        dvb, _gst_intern_string(varNode->v_list.name));
    OBJ_DEFERRED_VARIABLE_BINDING_SET_CLASS(dvb,
                                            _gst_current_parser->currentClass);
    OBJ_DEFERRED_VARIABLE_BINDING_SET_DEFAULT_DICTIONARY(
        dvb, _gst_get_undeclared_dictionary());
    ...
    if (varNode) {
      ...
      OOP arrayOOP;
      gst_object array = instantiate_with(_gst_array_class, size, &arrayOOP);
...
OOP make_block(int args, int temps, bc_vector bytecodes, int stack_depth) {
  OOP blockOOP;
  ...
  blockOOP = _gst_block_new(args, temps, bytecodes, stack_depth,
                            _gst_compiler_state->literal_vec);
...
OOP make_clean_block_closure(OOP blockOOP) {
...
OOP compute_selector(tree_node selectorExpr) {
...
OOP _gst_compute_keyword_selector(tree_node keywordList) {
  tree_node keyword;
  int len;
  char *nameBuf, *p;

  len = 0;
  for (keyword = keywordList; keyword != NULL; keyword = keyword->v_list.next) {
...
OOP get_attributes_array(tree_node attribute_list) {
  OOP attributesOOP;
  gst_object attributes;
  int n, i;

  n = _gst_list_length(attribute_list);
  attributes = new_instance_with(_gst_array_class, n, &attributesOOP);

  for (i = 0; attribute_list;
       attribute_list = attribute_list->v_list.next, i++) {
...
void record_attribute(OOP messageOOP) {
  method_attributes *new_attr =
      (method_attributes *)xmalloc(sizeof(method_attributes));
...
int add_literal(OOP oop) {
  int i;
  if (_gst_compiler_state->literal_vec_curr >=
      _gst_compiler_state->literal_vec_max) {
    realloc_literal_vec();
  }
...
int _gst_add_forced_object(OOP oop) {
  OOP *lit;
...
OOP get_literals_array(void) {
  ...
  methodLiterals = new_instance_with(_gst_array_class, n, &methodLiteralsOOP);
...
void install_method(OOP methodOOP, OOP classOOP) {
  ...
  for (i = 0; i < num_attrs; i++) {
    ...
    OOP handlerBlockOOP =
        _gst_find_pragma_handler(classOOP, OBJ_MESSAGE_GET_SELECTOR(attribute));

    if (!IS_NIL(handlerBlockOOP)) {
      _gst_msg_sendf(&result, "%s %o value: %o value: %o", handlerBlockOOP,
                     methodOOP, attributeOOP);
      if (result != NULL) {
  ...
  /* methodDictionaryOOP is held onto by the class, which is already
     ...
     here.  */
  methodDictionaryOOP = _gst_valid_class_method_dictionary(classOOP);
...
OOP _gst_make_new_method(int numArgs, int numTemps, int maximumStackDepth,
                         OOP attributesOOP, OOP literalsOOP,
                         bc_vector bytecodes, OOP class, OOP selector,
                         OOP categoryOOP, int64_t startPos, int64_t endPos) {
  ...
  methodDesc =
      method_info_new(class, selector, method_attrs, sourceCode, categoryOOP);
...
OOP _gst_block_new(int numArgs, int numTemps, bc_vector bytecodes,
                   int maximumStackDepth, OOP *literals) {
  int numByteCodes;
  OOP blockOOP;
  gst_compiled_block block;
  block_header header;
...
OOP method_info_new(OOP class, OOP selector, method_attributes *attrs,
                    OOP sourceCode, OOP categoryOOP) {
  method_attributes *next;
  gst_method_info methodInfo;
  OOP methodInfoOOP;

  methodInfo = (gst_method_info)new_instance_with(
      _gst_method_info_class, attrs ? attrs->count + 1 : 0, &methodInfoOOP);
