 ...
 *
 * Copyright
 *1988,89,90,91,92,94,95,99,2000,2001,2002,2003,2005,2006,2007,2008,2009,2011
...
static int my_stat_old(const char *name, struct gst_stat_struct *out);
static int my_stat(const char *name, OOP out);
#ifdef HAVE_LSTAT
static int my_lstat_old(const char *name, struct gst_stat_struct *out);
static int my_lstat(const char *name, OOP out);
#endif
...
static inline int my_stat_old(const char *name, struct gst_stat_struct *out) {
  int result;
  struct stat statOut;
...
int my_stat(const char *name, OOP out) {
  int result;
  struct stat statOut;
...
#ifdef HAVE_LSTAT
static inline int my_lstat_old(const char *name, struct gst_stat_struct *out) {
  int result;
  struct stat statOut;
...
int my_lstat(const char *name, OOP out) {
  int result;
  struct stat statOut;
...
long long test_longlong(long long aVerylongInt) {
  printf("Getting a long long 0x%llx\n", aVerylongInt);
  return aVerylongInt;
}

void test_callin(OOP oop, int (*callback)(const char *)) {
  OOP o;
  double f;
  int i;
  ...
  _gst_str_msg_send(_gst_str_msg_send(o, ",", o, NULL), "printNl", NULL);
  i = callback("this is a test");
  _gst_msg_sendf(&f, "%f %i + %f", i, 4.7);
  printf("result = %f\n", f);
}

void test_cstring(char **string) { printf("The string is %s\n", *string); }

void test_cobject_ptr(const void **string) { *string = "this is a test"; }

char *extract_dirent_name(struct dirent *dir) { return (dir->d_name); }

int get_argc(void) { return (_gst_smalltalk_passed_argc); }

const char *get_argv(int n) {
...
void _gst_dladdsearchdir(const char *dir) {
  lt_dlinsertsearchdir(lt_dlgetsearchpath(), dir);
}

void _gst_dlpushsearchpath(void) {
...
PTR _gst_lookup_function(const char *funcName) {
...
void _gst_invalidate_croutine_cache(void) {
...
OOP _gst_invoke_croutine(OOP cFuncOOP, OOP receiver, OOP *args) {
  gst_c_callable desc;
  cdata_type cType;
  ...
  c_func_cur->args = local_arg_vec =
      (cparam *)alloca(totalArgs * sizeof(cparam));

  /* The ffi_cif holds a pointer to this, so we must malloc it.  */
  if (c_func_cur->types_size < totalArgs) {
    c_func_cur->types =
        (ffi_type **)realloc(c_func_cur->types, totalArgs * sizeof(ffi_type *));
  ...
  if (c_func_cur->cacheGeneration != cif_cache_generation) {
    ffi_prep_cif(&c_func_cur->cacheCif, FFI_DEFAULT_ABI, totalArgs,
                 get_ffi_type(desc->returnTypeOOP), c_func_cur->types);
...
ffi_type *smalltalk_to_c(OOP oop, cparam *cp, cdata_type cType) {
  ...
  if (cType == CDATA_UNKNOWN)
    cType =
        (oop == _gst_true_oop || oop == _gst_false_oop)
            ...
            : oop == _gst_nil_oop
                  ? CDATA_COBJECT
                  : class == _gst_char_class
                        ? CDATA_CHAR
                        : class == _gst_unicode_character_class
                              ? CDATA_WCHAR
                              : class == _gst_byte_array_class
                                    ? CDATA_BYTEARRAY
                                    : is_a_kind_of(class, _gst_integer_class)
                                          ? CDATA_LONG
                                          : is_a_kind_of(class,
                                                         _gst_string_class)
                                                ? CDATA_STRING
  ...
  if (cType == CDATA_OOP) {
    ...
    case CDATA_BOOLEAN:
      ...
      return &ffi_type_sint;
            ...
            (cType == CDATA_CHAR || cType == CDATA_UCHAR ||
             cType == CDATA_WCHAR)) ||
  ...
  else if (class == _gst_undefined_object_class) { /* how to encode nil */
    switch (cType) {
    case CDATA_COBJECT_PTR:
    case CDATA_COBJECT:
    case CDATA_BYTEARRAY:
...
OOP c_to_smalltalk(cparam *result, OOP receiverOOP, OOP returnTypeOOP) {
  ...
  default:
    _gst_errorf("Invalid C function return type specified, index %d\n",
                returnType);
...
void _gst_free_closure(OOP callbackOOP) {
...
void _gst_set_errno(int errnum) {
...
int my_chown(const char *file, const char *user, const char *group) {
