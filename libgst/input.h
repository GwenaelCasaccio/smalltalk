...
/* Pass file descriptor FD, printed as file name FILENAME, to the
   parser.  */
extern void _gst_push_unix_file(int fd, const char *fileName) ATTRIBUTE_HIDDEN;

/* Return the current line and column into Y and X, respectively.
   Also sets the method_start_pos pointer.  */
extern YYLTYPE _gst_get_location(void) ATTRIBUTE_HIDDEN;

/* Pass the OOP to the parser; it must respond to #nextHunk.  */
extern void _gst_push_stream_oop(OOP oop) ATTRIBUTE_HIDDEN;
...
extern void _gst_push_smalltalk_string(OOP stringOOP) ATTRIBUTE_HIDDEN;
...
/* Pass the contents of the null-terminated String STRING to the
   ...
   is saved.  */
extern void _gst_push_cstring(const char *string) ATTRIBUTE_HIDDEN;
...
/* Pass the contents of stdin (either through the readline interface
   ...
   a tty.  */
extern void _gst_push_stdin_string(void) ATTRIBUTE_HIDDEN;
...
extern void _gst_set_stream_info(int line, OOP fileOOP, OOP fileNameOOP,
                                 int fileOffset) ATTRIBUTE_HIDDEN;
...
extern void _gst_warningf_at(int line, const char *str,
                             ...) ATTRIBUTE_PRINTF_2 ATTRIBUTE_HIDDEN;
...
/* Emits an error, with the current file and the given LINE in front
   of it, on the standard error descriptor.  */
extern void _gst_errorf_at(int line, const char *str,
                           ...) ATTRIBUTE_PRINTF_2 ATTRIBUTE_HIDDEN;

/* Interface to the Bison-generated parser.  */
extern void _gst_yyerror(const char *s) ATTRIBUTE_HIDDEN;
...
extern void _gst_unread_char(int ic) ATTRIBUTE_HIDDEN;
...
extern stream_type
_gst_get_cur_stream_type(void) ATTRIBUTE_PURE ATTRIBUTE_HIDDEN;
...
/* Returns an OOP containing the string of the topmost stream if it is
   ...
   nil otherwise.  */
extern OOP _gst_get_source_string(off_t startPos,
                                  off_t endPos) ATTRIBUTE_HIDDEN;

#ifdef HAVE_READLINE
/* Initialize the completion interface for Readline.  */
extern void _gst_initialize_readline(void) ATTRIBUTE_HIDDEN;
...
extern void _gst_enable_completion(void) ATTRIBUTE_HIDDEN;
...
extern void _gst_disable_completion(void) ATTRIBUTE_HIDDEN;

#endif /* HAVE_READLINE */

/* Parse the Smalltalk source code read from stdin, showing the
   PROMPT that is passed.  */
extern void _gst_process_stdin(const char *prompt) ATTRIBUTE_HIDDEN;
