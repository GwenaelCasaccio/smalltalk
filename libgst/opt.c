...
#define ALLOCA_BASIC_BLOCK(dest, depth, bp_, sp_)                              \
  do {                                                                         \
    *(dest) = alloca(sizeof(basic_block_item) + sizeof(OOP) * ((depth)-1));    \
    (*(dest))->bb = (dest);                                                    \
    (*(dest))->bp = (bp_);                                                     \
    (*(dest))->sp = (sp_);                                                     \
...
/* Use the hash table and function in superop1.inl to look for a
   ...
   by bytecode BC2.  */
static inline int search_superop_fixed_arg_1(int bc1, int arg, int bc2);

/* Use the hash table and function in superop1.inl to look for a
   superoperator representing bytecode BC1 followed by bytecode BC2
   with argument ARG.  */
static inline int search_superop_fixed_arg_2(int bc1, int bc2, int arg);
...
/* Scan the bytecodes between FROM and TO, performing a handful of
   ...
   starting at FROM.  */
static int optimize_basic_block(gst_uchar *from, gst_uchar *to);
...
static gst_uchar *optimize_superoperators(gst_uchar *from, gst_uchar *to);
...
int _gst_is_simple_return(bc_vector bytecodes, OOP *literalOOP) {
  ...
  *literalOOP = NULL;
  ...
  for (i = 1; i <= 4; i++) {
    ...
    MATCH_BYTECODES(
        IS_SIMPLE_RETURN, bytes,
        (
            PUSH_SELF { maybe = MTH_RETURN_SELF; } PUSH_RECEIVER_VARIABLE {
              maybe = (n << 8) | MTH_RETURN_INSTVAR;
            } PUSH_LIT_CONSTANT {
              maybe = (n << 8) | MTH_RETURN_LITERAL;
            } PUSH_INTEGER {
              maybe_object = FROM_INT(n);
              maybe = MTH_RETURN_LITERAL;
            } PUSH_SPECIAL {
              maybe = MTH_RETURN_LITERAL;
              switch (n) {
              case NIL_INDEX:
                maybe_object = _gst_nil_oop;
                break;
              case TRUE_INDEX:
                maybe_object = _gst_true_oop;
                break;
              case FALSE_INDEX:
                maybe_object = _gst_false_oop;
                break;
              default:
                abort();
              }
            ...
            LINE_NUMBER_BYTECODE {} RETURN_CONTEXT_STACK_TOP {
              if (maybe_object)
            ...
            PUSH_OUTER_TEMP, STORE_OUTER_TEMP, JUMP, POP_JUMP_TRUE,
            POP_JUMP_FALSE, PUSH_TEMPORARY_VARIABLE, PUSH_LIT_VARIABLE,
            STORE_TEMPORARY_VARIABLE, STORE_LIT_VARIABLE, SEND,
            POP_INTO_NEW_STACKTOP, POP_STACK_TOP, DUP_STACK_TOP, SEND_IMMEDIATE,
            EXIT_INTERPRETER, SEND_ARITH, SEND_SPECIAL, MAKE_DIRTY_BLOCK,
...
int _gst_check_kind_of_block(bc_vector bc, OOP *literals) {
  ...
  for (bp = bc->base, end = bc->ptr; bp != end;) {
    MATCH_BYTECODES(
        ...
        (
            ...
            JUMP,
            POP_JUMP_TRUE, POP_JUMP_FALSE, PUSH_TEMPORARY_VARIABLE,
            PUSH_LIT_VARIABLE, PUSH_SPECIAL, PUSH_INTEGER,
            RETURN_CONTEXT_STACK_TOP, LINE_NUMBER_BYTECODE,
            STORE_TEMPORARY_VARIABLE, STORE_LIT_VARIABLE, SEND,
            POP_INTO_NEW_STACKTOP, POP_STACK_TOP, DUP_STACK_TOP,
            EXIT_INTERPRETER, SEND_ARITH, SEND_SPECIAL, SEND_IMMEDIATE,
            MAKE_DIRTY_BLOCK {}

            RETURN_METHOD_STACK_TOP { return (31); } INVALID { abort(); }));
  }
  return (status);
}

int check_inner_block(OOP blockOOP) {
  int newStatus;
...
int compare_blocks(const PTR a, const PTR b) {
...
int search_block(const PTR a, const PTR b) {
...
int compute_jump_length(int ofs) {
...
bc_vector _gst_optimize_bytecodes(bc_vector bytecodes) {
#ifdef NO_OPTIMIZE
  return (bytecodes);
#else
  ...
  for (last = blocks; bp != end;) {
    ...
    do {
      ...
      MATCH_BYTECODES(
          THREAD_JUMPS, dest,
          (
              MAKE_DIRTY_BLOCK, SEND_SPECIAL, SEND_ARITH, SEND_IMMEDIATE,
              PUSH_RECEIVER_VARIABLE, PUSH_TEMPORARY_VARIABLE,
              PUSH_LIT_CONSTANT, PUSH_LIT_VARIABLE, PUSH_SELF, PUSH_SPECIAL,
              PUSH_INTEGER, LINE_NUMBER_BYTECODE, STORE_RECEIVER_VARIABLE,
              STORE_TEMPORARY_VARIABLE, STORE_LIT_VARIABLE, SEND,
              POP_INTO_NEW_STACKTOP, POP_STACK_TOP, DUP_STACK_TOP,
              ...
              JUMP {
                if (ofs == 2 && dest[0] == LINE_NUMBER_BYTECODE) {
                  ...
                  dest[-2] = dest[0];
                  dest[-1] = dest[1];
                } else if (ofs == 4 && IS_PUSH_BYTECODE(dest[0]) &&
              ...
              POP_JUMP_FALSE {
                ...
                /* Jumps to CONDITIONAL jumps must not be touched, either
                   because
    ...
    if (split) {
      assert(bp[-2] == kind);
      assert(kind == JUMP || kind == JUMP_BACK || kind == POP_JUMP_TRUE ||
             kind == POP_JUMP_FALSE);
  ...
  for (block = blocks; block != last; block++) {
    ...
    if (bp[-2] == JUMP || bp[-2] == JUMP_BACK || bp[-2] == POP_JUMP_TRUE ||
        bp[-2] == POP_JUMP_FALSE) {
      do {
  ...
  for (block = blocks; block != last; block++) {
    ...
    if (block->kind != -1 && block->opt_length > 0 &&
        !(block->opt_length > 2 && first[block->opt_length - 4] == EXT_BYTE)) {
      int test =
          search_superop_fixed_arg_1(first[block->opt_length - 2],
                                     first[block->opt_length - 1], block->kind);
      if (test != -1) {
        block->opt_length -= 2;
        block->kind = test;
      }
  ...
  for (block = blocks; block != last; block++) {
    ...
    if (block->kind != -1) {
      int jump_length = compute_jump_length(
          block->dest_bb->final_byte - (block->final_byte + block->opt_length));
...
int optimize_basic_block(gst_uchar *from, gst_uchar *to) {
  ...
  do {
    ...
    case PUSH_RECEIVER_VARIABLE:
      ...
      if (opt >= from + 4 && (opt == from + 4 || opt[-6] != EXT_BYTE) &&
          opt[-4] ==
              bp[0] + (STORE_TEMPORARY_VARIABLE - PUSH_TEMPORARY_VARIABLE) &&
          opt[-3] == bp[1] && opt[-2] == POP_STACK_TOP && bp[-2] != EXT_BYTE) {
        opt -= 2;
        bp += 2;
        continue;
      }
      ...
      if (opt >= from + 6 && (opt == from + 6 || opt[-8] != EXT_BYTE) &&
          opt[-6] ==
              bp[0] + (STORE_TEMPORARY_VARIABLE - PUSH_TEMPORARY_VARIABLE) &&
          opt[-5] == bp[1] && opt[-4] == POP_STACK_TOP &&
          opt[-2] == LINE_NUMBER_BYTECODE && bp[-2] != EXT_BYTE) {
        opt[-4] = opt[-2];
        opt[-3] = opt[-1];
        opt -= 2;
        bp += 2;
    ...
    case PUSH_LIT_CONSTANT:
      ...
      if (bp < to - 2 && bp[2] == POP_STACK_TOP) {
        bp += 4;
        continue;
      }

      /* Remove the pop in a pop/push/return sequence */
      if (opt >= from + 2 && bp < to - 2 && bp[2] == RETURN_CONTEXT_STACK_TOP &&
          opt[-2] == POP_STACK_TOP) {
        opt -= 2;
      }
      ...
      if (opt >= from + 4 && opt[-4] == POP_STACK_TOP &&
...
gst_uchar *optimize_superoperators(gst_uchar *from, gst_uchar *to) {
...
void _gst_compute_stack_positions(gst_uchar *bp, int size, PTR *base,
                                  PTR **pos) {
      ...
      MATCH_BYTECODES(
          ...
          (
              ...
              RETURN_CONTEXT_STACK_TOP {
                ...
                if (!bb_start[bc_len]) {
                  ALLOCA_BASIC_BLOCK(bb_start + bc_len, 0, bp_first + bc_len,
                                     curr_sp + balance);
              ...
              PUSH_TEMPORARY_VARIABLE, PUSH_LIT_CONSTANT, PUSH_LIT_VARIABLE,
              PUSH_SELF, PUSH_SPECIAL, PUSH_INTEGER, DUP_STACK_TOP,
              PUSH_OUTER_TEMP { balance++; }

              LINE_NUMBER_BYTECODE,
              STORE_RECEIVER_VARIABLE, STORE_TEMPORARY_VARIABLE,
              STORE_LIT_VARIABLE, STORE_OUTER_TEMP, EXIT_INTERPRETER,
              MAKE_DIRTY_BLOCK {}

              SEND { balance -= super + num_args; }
              ...
              JUMP {
                ...
                if (!bb_start[ofs]) {
                  ALLOCA_BASIC_BLOCK(bb_start + ofs, 0, bp_first + ofs,
                                     curr_sp + balance);
...
void _gst_analyze_bytecodes(OOP methodOOP, int size, char *dest) {
  gst_uchar *bp;
...
int make_destination_table(gst_uchar *bp, int size, char *dest) {
  gst_uchar *end, *bp_first;
  int count;

  memset(dest, 0, sizeof(char) * size);

  for (count = 0, end = bp + size; bp != end; dest += bp - bp_first) {
    bp_first = bp;
    MATCH_BYTECODES(MAKE_DEST_TABLE, bp,
                    (
                        ...
                        RETURN_CONTEXT_STACK_TOP, LINE_NUMBER_BYTECODE,
                        STORE_RECEIVER_VARIABLE, STORE_TEMPORARY_VARIABLE,
                        STORE_LIT_VARIABLE, SEND, POP_INTO_NEW_STACKTOP,
                        POP_STACK_TOP, DUP_STACK_TOP, PUSH_OUTER_TEMP,
                        STORE_OUTER_TEMP, EXIT_INTERPRETER, SEND_ARITH,
                        SEND_SPECIAL, SEND_IMMEDIATE,
                        MAKE_DIRTY_BLOCK {}
...
#define LITERAL_CLASS(n) OOP_INT_CLASS(literals[(n)])
...
void _gst_verify_sent_method(OOP methodOOP) {
  ...
  if (error) {
    ...
    _gst_errorf("Method verification failed for %O>>%O",
                GET_METHOD_CLASS(methodOOP), GET_METHOD_SELECTOR(methodOOP));

    abort();
  }
}

const char *_gst_verify_method(OOP methodOOP, int *num_outer_temps, int depth) {
#ifndef NO_VERIFIER
  int size, bc_len, num_temps, stack_depth, num_literals, num_rec_vars,
      num_ro_rec_vars;
    ...
    do {
      ...
      MATCH_BYTECODES(
          ...
          (
              ...
              PUSH_RECEIVER_VARIABLE,
              PUSH_TEMPORARY_VARIABLE, PUSH_LIT_CONSTANT, PUSH_LIT_VARIABLE,
              PUSH_SELF, PUSH_SPECIAL, PUSH_INTEGER,
              PUSH_OUTER_TEMP { balance++; }
              ...
              DUP_STACK_TOP { balance++; }

              INVALID { return ("invalid bytecode"); }
          ...
          (
              ...
              PUSH_SELF {
                ...
                *sp++ = FROM_INT(SELF);
                ...
                switch (n) {
                case NIL_INDEX:
                  last_used_literal = _gst_nil_oop;
                  break;
                case TRUE_INDEX:
                  last_used_literal = _gst_true_oop;
                  break;
                case FALSE_INDEX:
                  last_used_literal = _gst_false_oop;
                  break;
                default:
                  return "invalid special object index";
                }
              ...
              } STORE_TEMPORARY_VARIABLE { CHECK_TEMP(n); } STORE_LIT_VARIABLE {
                CHECK_LIT_VARIABLE(true, n);
              }

              SEND {
                if (super && (!last_used_literal ||
              ...
              PUSH_OUTER_TEMP {
                if (scopes == 0 || scopes > depth ||
                    n >= num_outer_temps[scopes])
                  return ("temporary out of range");
              ...
              EXIT_INTERPRETER {
                if (size != 4 || IP0 != GET_METHOD_BYTECODES(methodOOP) ||
              ...
              SEND_IMMEDIATE {
                if (n == NEW_COLON_SPECIAL && IS_INT(last_used_literal) &&
                    last_used_literal != FROM_INT(0) &&
                    sp[-2] == OOP_CLASS(_gst_array_class)) {
                  partially_constructed_array *a;
                  sp--;

                  /* If possible, reuse an existing struct, else allocate a new
                   * one.  */
                  if (arrays_pool) {
                    a = arrays_pool;
              ...
              MAKE_DIRTY_BLOCK {
                if (sp[-1] != _gst_compiled_block_class || !last_used_literal)
                  return ("CompiledBlock expected");

                error = _gst_verify_method(last_used_literal, num_outer_temps,
                                           depth);
                if (error)
                  return (error);
              }
