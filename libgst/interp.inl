...
#define SEND_MESSAGE(sendSelector, sendArgs)                                   \
  do {                                                                         \
    OOP _receiver;                                                             \
...
OOP tagged_and(OOP op1, OOP op2) {
  intptr_t iop1 = (intptr_t)op1;
  intptr_t iop2 = (intptr_t)op2;
  return (OOP)(iop1 & iop2);
}

OOP tagged_or(OOP op1, OOP op2) {
  intptr_t iop1 = (intptr_t)op1;
  intptr_t iop2 = (intptr_t)op2;
  return (OOP)(iop1 | iop2);
}

OOP tagged_xor(OOP op1, OOP op2) {
  intptr_t iop1 = (intptr_t)op1;
...
OOP add_with_check(OOP op1, OOP op2, bool *overflow) {
  ...
  intptr_t iresult;
    ...
    !defined NO_OPTIMIZED_SMALLINTEGERS
  int of = 0;
  iop2--;
  asm("add" OP_SUFFIX " %3, %2\n"
      "seto %b1"
      : "=r"(iresult), "+&q"(of)
      : "0"(iop1), OP_CONSTRAINT(iop2));
...
OOP mul_with_check(OOP op1, OOP op2, bool *overflow) {
  ...
  *overflow = false;
  ...
  /* This fallback method uses a division to do overflow check */
    ...
    if COMMON ((((uintptr_t)(a | b)) < (1L << (ST_INT_SIZE / 2)) || b == 0 ||
                result / b == a) &&
               !INT_OVERFLOW(result))
      return FROM_INT(result);
    else
...
uint32_t random_next(uint32_t *mt) {
...
void random_generate(uint32_t *mt) {
  const int N = RANDOM_SIZE - 1;
  const int M = 397;
  int i;
  for (i = 0; i < N; i++) {
